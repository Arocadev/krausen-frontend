...
export default function EditarCervezaPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/api/cervezas/${id}`).then((res) => {
      ...
      setImagenActual(c.imagen_url || null);
      setFilas(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imagenFile) {
        ...
        await api.post(`/api/cervezas/${id}/imagen`, formData, {
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] bg-espuma px-4 py-10 sm:px-6 sm:py-14">
      <div className="mx-auto max-w-2xl rounded-lg border border-linea bg-white p-6 sm:p-10">
        ...
        <p className="mt-2 text-tostado">Corrige los datos de tu elaboración.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-6">
          ...
          <div>
            ...
            <textarea rows={3} value={descripcion} onChange={(e) => setDescripcion(e.target.value)} className={inputClase} placeholder="Describe brevemente tu receta…" />
          </div>
          ...
          <div>
            ...
            {(imagenPreview || imagenActual) ? (
              <div className="relative">
                <img
                  src={imagenPreview || `http://127.0.0.1:8000${imagenActual}`}
                  alt="Preview"
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={agregarIngrediente} className="text-sm font-medium text-ambar-oscuro hover:underline">+ Añadir</button>
            </div>
            {filas.map((fila, i) => (
              <div key={i} className="mt-3 flex gap-2">
                ...
                <input type="number" step="0.1" min="0" placeholder="Cant." value={fila.cantidad} onChange={(e) => actualizarFila(i, "cantidad", e.target.value)} className="w-24 rounded-md border border-linea bg-white px-3 py-2 text-sm text-malta outline-none focus:border-ambar" />
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={agregarPaso} className="text-sm font-medium text-ambar-oscuro hover:underline">+ Añadir</button>
            </div>
            {pasos.map((paso, i) => (
              <div key={i} className="mt-3 flex gap-2">
                ...
                <input type="text" placeholder="Describe el paso…" value={paso.descripcion} onChange={(e) => actualizarPaso(i, "descripcion", e.target.value)} className="flex-1 rounded-md border border-linea bg-white px-3 py-2 text-sm text-malta outline-none focus:border-ambar" />
                <input type="number" min="0" placeholder="min" value={paso.duracion_min} onChange={(e) => actualizarPaso(i, "duracion_min", e.target.value)} className="w-20 rounded-md border border-linea bg-white px-3 py-2 text-sm text-malta outline-none focus:border-ambar" />
