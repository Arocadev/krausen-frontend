...
function FormularioCerveza() {
  ...
  useEffect(() => {
    if (forkId) {
      api.get(`/api/cervezas/${forkId}`).then((res) => setNombreOriginal(res.data.nombre)).catch(() => {});
    }
  }, [forkId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imagenFile) {
        ...
        await api.post(`/api/cervezas/${cervezaId}/imagen`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      }
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-espuma px-4 py-10 sm:px-6 sm:py-14">
      <div className="mx-auto max-w-2xl rounded-lg border border-linea bg-white p-6 sm:p-10">
        <h1 className="font-[family-name:var(--font-lora)] text-3xl font-semibold text-malta">
          {forkId ? t("forkTitulo") : t("nuevaTitulo")}
        </h1>
        <p className="mt-2 text-tostado">{forkId ? t("forkSubtitulo") : t("nuevaSubtitulo")}</p>
        {nombreOriginal && (
          <p className="mt-3 rounded-md bg-ambar/10 px-4 py-3 text-sm text-tostado">
            {t("basadaEn")}: <span className="font-medium text-malta">{nombreOriginal}</span>
          </p>
        )}
          ...
          <div>
            ...
            {imagenPreview ? (
              <div className="relative">
                <img src={imagenPreview} alt="Preview" className="h-48 w-full rounded-lg border border-linea object-cover" />
                <button type="button" onClick={() => { setImagenPreview(null); setImagenFile(null); if (inputImagenRef.current) inputImagenRef.current.value = ""; }} className="absolute right-2 top-2 rounded-full bg-malta/70 px-2 py-0.5 text-xs text-white hover:bg-malta">
                  {t("quitarImagen")}
                </button>
              </div>
            ) : (
