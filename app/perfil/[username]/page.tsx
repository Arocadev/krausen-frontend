...
export default function PerfilPublicoPage() {
  ...
  if (cargando) return <p className="py-24 text-center text-tostado">Cargando perfil…</p>;

  if (!perfil) return (
    <main className="mx-auto max-w-3xl px-6 py-24 text-center">
      <h1 className="font-[family-name:var(--font-lora)] text-2xl text-malta">Usuario no encontrado</h1>
      <Link href="/" className="mt-4 inline-block text-ambar-oscuro hover:underline">Volver al inicio</Link>
    </main>
  );
  ...
  const TarjetaCerveza = ({ c }: { c: Cerveza }) => (
    <Link
      href={`/cervezas/${c.id}`}
      className="group rounded-lg border border-linea bg-white p-5 transition-all hover:border-ambar/50 hover:shadow-[0_2px_12px_rgba(92,58,33,0.08)]"
    >
      {c.imagen_url && (
        <img
          src={`http://127.0.0.1:8000${c.imagen_url}`}
          alt={c.nombre}
      ...
      <h3 className="mt-3 font-[family-name:var(--font-lora)] text-lg font-semibold text-malta group-hover:text-ambar-oscuro">
        {c.nombre}
      </h3>
      {c.descripcion && (
      ...
      <div className="flex items-start gap-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-tostado">
            Cervecero desde {new Date(perfil.created_at).toLocaleDateString("es-ES", { month: "long", year: "numeric" })}
          </p>
          <div className="mt-3 flex gap-6 text-sm">
            <span className="text-tostado">
              <span className="font-semibold text-malta">{perfil.total_recetas}</span> recetas
            </span>
            <span className="text-tostado">
              <span className="font-semibold text-malta">{perfil.total_likes_recibidos}</span> me gustas recibidos
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-10 flex border-b border-linea">
        <button
          ...
          className={`px-4 pb-3 text-sm font-medium transition-colors ${
            tab === "recetas"
              ? "border-b-2 border-ambar text-ambar-oscuro"
              : "text-tostado hover:text-malta"
          }`}
        >
          Recetas ({perfil.recetas.length})
        </button>
        <button
          onClick={() => setTab("me_gustas")}
          className={`px-4 pb-3 text-sm font-medium transition-colors ${
            tab === "me_gustas"
              ? "border-b-2 border-ambar text-ambar-oscuro"
              : "text-tostado hover:text-malta"
        ...
        >
          Me gustas ({perfil.me_gustas.length})
        </button>
      </div>
      ...
      {lista.length === 0 ? (
        <div className="mt-10 rounded-lg border border-dashed border-linea bg-white py-16 text-center">
          <p className="font-[family-name:var(--font-lora)] text-xl text-malta">
            {tab === "recetas" ? "Aún no ha publicado recetas" : "Aún no ha dado me gustas"}
          </p>
        </div>
      ) : (
