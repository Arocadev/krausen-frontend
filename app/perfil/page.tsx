...
export default function PerfilPage() {
  ...
  useEffect(() => {
    if (usuario) api.get("/api/perfil/").then((res) => setPerfil(res.data)).catch(() => {});
  }, [usuario]);
        ...
        <div className="mt-8 rounded-lg border border-linea bg-white p-8">
          <div className="space-y-4">
            <div className="flex justify-between">
              <span className="text-sm text-tostado">{t("usuario")}</span>
              <span className="font-medium text-malta">{perfil.username}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-tostado">{t("email")}</span>
              <span className="font-medium text-malta">{perfil.email}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-tostado">{t("miembroDesde")}</span>
              <span className="font-medium text-malta">{new Date(perfil.created_at).toLocaleDateString("es-ES")}</span>
            </div>
          </div>
        </div>

        <div className="mt-8 rounded-lg border border-linea bg-white p-8">
          <h2 className="font-[family-name:var(--font-lora)] text-xl font-semibold text-malta">{t("cambiarPassword")}</h2>
          <form onSubmit={handlePassword} className="mt-6 flex flex-col gap-5">
            <PasswordInput id="pass-actual" label={t("passwordActual")} value={passActual} onChange={setPassActual} />
            <PasswordInput id="pass-nueva" label={t("nuevaPassword")} value={passNueva} onChange={setPassNueva} />
            <PasswordInput id="pass-confirmar" label={t("repiteNueva")} value={passConfirmar} onChange={setPassConfirmar} />
            {error && <p className="rounded-md bg-red-50 px-4 py-3 text-sm text-red-800">{error}</p>}
            {mensaje && <p className="rounded-md bg-green-50 px-4 py-3 text-sm text-green-800">{mensaje}</p>}
