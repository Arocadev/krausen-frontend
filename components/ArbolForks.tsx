...
function NodoArbol({ nodo, esRaiz }: { nodo: Nodo; esRaiz: boolean }) {
  return (
    <div className="flex flex-col items-center">
      <Link
        href={`/cervezas/${nodo.id}`}
        className={`rounded-lg border px-5 py-3 text-center transition-all hover:shadow-md ${
          nodo.es_actual
      ...
      >
        <p className={`font-[family-name:var(--font-lora)] text-sm font-semibold leading-tight ${
          nodo.es_actual ? "text-ambar-oscuro" : "text-malta"
        }`}>
          {nodo.nombre}
        </p>
        {nodo.username && (
          <p className="mt-1 text-xs text-tostado">por {nodo.username}</p>
        )}
        {esRaiz && (
      ...
      {nodo.hijos.length > 0 && (
        <>
          ...
          <div className="relative flex gap-8">
            {nodo.hijos.length > 1 && (
              <div
                className="absolute top-0 h-px bg-linea"
                style={{
                  left: "50%",
                  right: "50%",
                  marginLeft: `-${(nodo.hijos.length - 1) * 52}px`,
                  marginRight: `-${(nodo.hijos.length - 1) * 52}px`,
                }}
              />
            )}
            {nodo.hijos.map((hijo) => (
              <div key={hijo.id} className="flex flex-col items-center">
                <div className="h-6 w-px bg-linea" />
                <NodoArbol nodo={hijo} esRaiz={false} />
              </div>
            ))}
          </div>
        </>
...
export default function ArbolForks({ cervezaId }: { cervezaId: string }) {
  ...
  if (arbol.hijos.length === 0 && !arbol.es_actual) return null;

  const tieneArbol = arbol.hijos.length > 0 || arbol.es_actual;
  if (!tieneArbol) return null;
    ...
    <section className="border-t border-linea">
      <div className="mx-auto max-w-4xl px-6 py-14">
        ...
        <div className="mt-8 flex justify-center overflow-x-auto py-4">
          <NodoArbol nodo={arbol} esRaiz={true} />
        </div>
      </div>
    </section>
  );
